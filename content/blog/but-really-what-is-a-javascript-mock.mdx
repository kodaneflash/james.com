...
meta:
  keywords:
    ...
    - testing
    - mocking
bannerCloudinaryId: kentcdodds.com/content/blog/but-really-what-is-a-javascript-mock/banner
...
test('returns winner', () => {
	const winner = thumbWar('Ken Wheeler', 'James Archer')
	expect(['Ken Wheeler', 'James Archer'].includes(winner)).toBe(true)
})
```
...
test('returns winner', () => {
	...
	const winner = thumbWar('Ken Wheeler', 'James Archer')
	expect(winner).toBe('James Archer')
...
test('returns winner', () => {
	...
	utils.getWinner.mock.calls.forEach((args) => {
		expect(args).toEqual(['Ken Wheeler', 'James Archer'])
	})
